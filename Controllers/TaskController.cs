                ...
                { // TODO 設備異常
                    ...
                    else if (action == ACTION_TYPE.Load)
                    {
                        ...
                        if (result.mainEQ.Load_Request == false)
                            return Ok(new clsAGVSTaskReportResponse() { confirm = false, AlarmCode = ALARMS.EQ_LOAD_REQUEST_IS_NOT_ON, message = $"Load_Request={result.mainEQ.Unload_Request} cannot Unload" });
